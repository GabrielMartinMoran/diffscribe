...
function mkTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'diffscribe-test-fde-'));
}
...
describe('File Diff endpoint (integration)', () => {
  let tempDir: string;
  let repoDir: string;
  let baseUrl: string;
  ...
  beforeEach(async () => {
    ...
    // If no test URL, we skip endpoint tests
    baseUrl = appUrl;
  });
  ...
  it('rejects requests to workspace that does not exist', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/workspaces/nonexistent-id/file-diff?comparison=${encodedComparison}&path=${encodedPath}`,
    );
  ...
  it('rejects missing comparison parameter', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/workspaces/nonexistent-id/file-diff?path=${encodedPath}`,
    );
  ...
  it('rejects missing path parameter', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/workspaces/nonexistent-id/file-diff?comparison=${encodedComparison}`,
    );
    expect(response.status).toBe(400);
  });
  ...
  it('rejects absolute paths (path traversal)', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/workspaces/some-id/file-diff?comparison=${encodedComparison}&path=${encodedPath}`,
    );
    expect(response.status).toBe(400);
  });

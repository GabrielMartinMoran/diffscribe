...
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    const { listUseCase, getReviewUseCase, markFileUseCase } = getServices();
    ...
    const reviewedAt = await markFileUseCase.execute({
      workspaceId: params.id,
      reviewId: params.reviewId,
      filePath,
    });

    return json({ reviewedAt: reviewedAt.toISOString() });
  } catch (error) {
